...
/**
 * Launches the first task, the kernel task
 * @param tss_selector The tss selector for the task
 */
extern void task_ltr(uint16_t tss_selector);

/**
 * Launches the task specified by the selector
 * @param tss_selector The tss selector for the task
 */
...
/**
 * Creates an LDT entry with the specified parameters, can only be set in the GDT!
 * @param base The base of the LDT
 * @param limit The limit of the LDT
 * @param dpl The privilege level
 * @return A LDT entry for the task
 */
...
/**
 * Initialises the task for the id passed in argument
 * @param id The id of the task to init
 */
static void task_init(uint id) {
	...
	gdt_table[GDT_KERNEL_ENTRIES + id * 2 + 1] = gdt_make_ldt((uint32_t) tasks[id].task_ldt,
															  sizeof(tasks[id].task_ldt) - 1, DPL_KERNEL);
	...
	tasks[id].task_ldt[tasks[id].ldt_code_idx] = gdt_make_code_segment((uint32_t) tasks[id].address,
																	   tasks[id].limit / 4096, DPL_USER);
	tasks[id].task_ldt[tasks[id].ldt_data_idx] = gdt_make_data_segment((uint32_t) tasks[id].address,
																	   tasks[id].limit / 4096, DPL_USER);
